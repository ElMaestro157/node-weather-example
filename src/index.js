    ...
    return (
      <div>
        <input type="text" onChange={this.onChange} value={text}/>
          ...
          {
            cities.length 
            ? (<span onClick={this.chooseHandler}>{cities[0]}</span>) 
            : null
          }
          </div>
      </div>
...
const Weather = props => props.weather ? (
  <div>
    <div>{`country: ${props.weather.country}`}</div>
    <div>{`temperature: ${props.weather.temperature}`}</div>
    <div>{`cloud: ${props.weather.cloud}`}</div>
    <div>{`condition: ${props.weather.condition}`}</div>
    <img src={props.weather.conditionIcon}/>
  </div>
  ) : null;
  ...
  class App extends React.Component {
    ...
    render() {
      return (
        <div>
          <Search chooseCityCallback={this.chooseCityCallback}/>
